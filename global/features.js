/* Wibe Stories — Features page scripts */
(function() {
  /* --- Theme --- */
  ...
  faqButtons.forEach(function(q) {
    ...
    q.addEventListener('keydown', function(e) {
      ...
      switch (e.key) {
        case 'ArrowDown': e.preventDefault(); if (idx < buttons.length - 1) buttons[idx + 1].focus(); break;
        case 'ArrowUp': e.preventDefault(); if (idx > 0) buttons[idx - 1].focus(); break;
        case 'Home': e.preventDefault(); buttons[0].focus(); break;
        case 'End': e.preventDefault(); buttons[buttons.length - 1].focus(); break;
        case 'Enter':
        case ' ': e.preventDefault(); toggleFaq(this); break;
      }
    });
  });
  function toggleFaq(btn) {
    ...
    document.querySelectorAll('.faq-item.open').forEach(function(otherItem) {
      if (otherItem !== item) {
        ...
        if (otherAnswer) { otherAnswer.style.maxHeight = '0'; otherAnswer.classList.remove('open'); }
      }
    });
    if (isOpen) {
      ...
      if (answer) { answer.style.maxHeight = '0'; answer.classList.remove('open'); }
    } else {
      item.classList.add('open');
      btn.classList.add('open');
      btn.setAttribute('aria-expanded', 'true');
      if (answer) { answer.classList.add('open'); answer.style.maxHeight = answer.scrollHeight + 'px'; }
