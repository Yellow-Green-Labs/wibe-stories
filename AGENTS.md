...
<!-- agsync: last-run 2026-06-21; Pro membership edge case mitigations: added api/resend-key.js for automated key recovery, updated handleUpgradeEmail() to call API instead of mailto, added dev bypass for WS-TEST-DEMO-KEY in api/pro-status.js and lib/pro-key.js, documented BMAC webhook setup in DEVELOPER.md. About page: replaced SVG values circle with bento grid layout (3-col desktop with 2x2 anchor card, 2-col mobile). -->
