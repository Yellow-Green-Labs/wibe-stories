...
// Get the issuer URL for token verification.
// Priority: CLERK_ISSUER_URL env var > derived from publishable key.
export function getClerkIssuerUrl() {
  const explicit = process.env.CLERK_ISSUER_URL;
  if (explicit) return explicit.replace(/\/$/, '');
  return getClerkInstanceUrl();
}
 ...
 *  2. `iss` must match CLERK_ISSUER_URL (production) or derived instance URL (development).
...
export async function verifyClerkToken(token, redis) {
  ...
  if (!issuerUrl || !secretKey) {
    console.warn('[Clerk] CLERK_ISSUER_URL/CLERK_PUBLISHABLE_KEY or CLERK_SECRET_KEY missing — verification disabled');
    return null;
  }
  ...
  try {
    ...
    // Issuer must match our configured issuer (production) or derived instance (dev)
    // Accept both during transition
    const derivedUrl = getClerkInstanceUrl();
    const validIssuers = [issuerUrl];
    if (derivedUrl && derivedUrl !== issuerUrl) validIssuers.push(derivedUrl);
    if (!peeked.iss || !validIssuers.includes(peeked.iss)) return null;
