 ...
 */
export async function createSessionToken(redis, proKey, tier, expiresAt, email, membershipType) {
  const token = crypto.randomUUID();
  const endOfDay = new Date();
  endOfDay.setUTCHours(23, 59, 59, 999);
  const ttlToKeyExpiry = expiresAt
    ? Math.max(60, Math.floor((new Date(expiresAt).getTime() - Date.now()) / 1000))
    : 30 * 24 * 60 * 60;
  const ttlToEndOfDay = Math.max(60, Math.floor((endOfDay.getTime() - Date.now()) / 1000));
  const ttlSeconds = Math.max(ttlToKeyExpiry, ttlToEndOfDay);
